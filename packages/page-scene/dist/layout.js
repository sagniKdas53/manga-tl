...
export function textFitBox(box, inset = DEFAULT_TEXT_BOX_INSET) {
...
export function clampLineCenter(center, lineWidth, boxX, boxWidth) {
...
function parsePolygon(maskPolygon) {
    ...
    try {
        ...
        if (Array.isArray(parsed) &&
            parsed.every((point) => Array.isArray(point) &&
                point.length === 2 &&
...
export function fitTextInBox({ text, maxWidth, maxHeight, fontFamily, defaultFontSize = 16, shape = "rectangular", boxX = 0, boxY = 0, maskPolygon, fontWeight = "bold", fontStyle = "normal", }, measureText) {
    ...
    const fontFor = (fontSize) => `${fontWeight} ${fontStyle === "italic" ? "italic " : ""}${fontSize}px "${fontFamily}", sans-serif`;
    const measure = (fontSize, value) => measureText(fontFor(fontSize), value);
        ...
        if (polygonPoints && polygonPoints.length > 0) {
            ...
            const spanAt = (lineCount, index) => {
                ...
                for (let pointIndex = 0; pointIndex < polygonPoints.length; pointIndex++) {
                    ...
                    if ((y1 <= lineCenterY && y2 > lineCenterY) ||
            ...
            const tryWrap = (lineCount) => {
                ...
                for (let paragraphIndex = 0; paragraphIndex < paragraphs.length; paragraphIndex++) {
                    ...
                    for (const word of paragraph.split(" ")) {
                        ...
                        if (measure(fontSize, word) > allowedWidth) {
                            ...
                            for (const character of word) {
                                ...
                                if (measure(fontSize, wordPart + character) >
                    ...
                    if (currentLine) {
                        ...
                        currentLine = "";
                        lineIndex++;
                        if (lineIndex >= lineCount && paragraphIndex < paragraphs.length - 1)
                            return null;
                    }
                }
            ...
            for (let lineCount = 1; lineCount <= Math.floor(maxHeight / lineHeight); lineCount++) {
                ...
                if (wrapped)
                    return { lines: wrapped.lines, lineCenters: wrapped.centers, failed: false };
            }
        }
    ...
    const evaluate = (fontSize) => {
        ...
        const widestLine = res.lines.reduce((widest, line) => Math.max(widest, measure(fontSize, line)), 0);
        const fitsHeight = res.lines.length * fontSize * LINE_HEIGHT_MULTIPLIER <= maxHeight;
        const unbroken = res.lines.join(" ").split(/\s+/).filter(Boolean).join(" ") ===
    ...
    const best = largestSizeWhere("clean") ??
