 ...
 */
export function resolvePageScene(input, measureText) {
    const diagnostics = [];
    const objects = [];
 ...
 */
export function renderPageSceneSvg(scene) {
    const { source } = scene.input;
    const resolvedById = new Map(scene.objects.map((object) => [object.objectId, object]));
    ...
    const glyphMarkup = [...scene.input.textObjects]
        ...
        if (!resolved || resolved.lineBoxes.length === 0)
            return "";
