...
- **1.2. Chapter-Level Memory Toggle**
  ...
  - **Tasks**:
    - Add a toggle (UI and backend) to enable/disable the injection of previous page context at the chapter level.
- **1.3. Eliminate Redundant Keys**
  - **Why**: Simple cleanup to reduce confusion.
  - **Tasks**:
    - Remove `NVIDIA_OCR_API_KEY` requirement and fall back entirely to `NVIDIA_API_KEY`.
- **1.4. Docker Secrets File Support**
  - **Why**: Best practice for secure deployments (Swarm/Compose).
  - **Tasks**:
    - Add support for the `_FILE` suffix convention in both backend and worker config loaders.
    - Enable reading secrets for Database, MinIO, JWT, and API Keys from mounted files (e.g., `/run/secrets/db_password`).
    - *Optional enhancement*: Support loading a single JSON file containing multiple secrets to avoid mounting many individual files.
