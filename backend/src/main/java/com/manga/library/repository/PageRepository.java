...
public interface PageRepository extends JpaRepository<Page, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT p.chapter.id, p.image.id FROM Page p WHERE p.chapter.series.id = :seriesId " +
           "AND p.pageNumber = (SELECT MIN(p2.pageNumber) FROM Page p2 WHERE p2.chapter.id = p.chapter.id)")
    List<Object[]> findFirstPageImageIdsBySeriesId(@org.springframework.data.repository.query.Param("seriesId") UUID seriesId);
}
