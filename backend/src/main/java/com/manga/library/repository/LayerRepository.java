...
public interface LayerRepository extends JpaRepository<Layer, UUID> {
   ...
   *  Used during clone to shift layers above the source up by +1.
   *  Uses explicit @Query because Spring Data's derived query parser mis-capitalises 'zOrder'
   *  as 'ZOrder' and cannot resolve the attribute automatically. */
  @Query("SELECT l FROM Layer l WHERE l.image.id = :imageId AND l.zOrder > :zOrder")
  List<Layer> findByImageIdAndZOrderGreaterThan(@Param("imageId") UUID imageId, @Param("zOrder") int zOrder);
}
