...
public class PageController {
  private static final org.slf4j.Logger log =
      org.slf4j.LoggerFactory.getLogger(PageController.class);
  ...
  public PageController(
      ...
      com.manga.library.service.PageService pageService,
      ...
      if (".zip".equalsIgnoreCase(fileExtension) || ".epub".equalsIgnoreCase(fileExtension)) {
        ...
        if (projectJsonBytes != null) {
          ...
          for (com.manga.library.dto.ZipImageEntry imgEntry : imageEntries) {
            ...
            jobCoordinatorService.startPipeline(
                pg.getImage().getId(), pg.getId(), chapter.getId());
            nextNum++;
          }
  ...
  public ResponseEntity<Map<String, Object>> getPage(@PathVariable UUID pageId) {
    ...
    List<UUID> conversationIds =
        conversations.stream().map(c -> c.getId()).collect(Collectors.toList());
