...
public class LayerController {
    ...
    public ResponseEntity<LayerElement> updateLayerElement(
            ...
            @RequestBody LayerElementDto dto,
            @AuthenticationPrincipal User user) {

        log.info("Updating LayerElement {} by user {}", id, user.getEmail());

        return layerElementRepository.findById(id)
                .map(element -> {
                    try {
                        ...
                        if (!prevJson.equals(newJson)) {
                            LayerEditHistory history = LayerEditHistory.builder()
    ...
    public ResponseEntity<List<LayerEditHistory>> getLayerElementHistory(@PathVariable UUID id) {
        ...
        List<LayerEditHistory> history = layerEditHistoryRepository.findByLayerElementIdOrderByEditedAtDesc(id);
