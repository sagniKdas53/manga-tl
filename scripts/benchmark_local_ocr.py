...
def draw_det_only(image, det_boxes, output_path, model_name):
    """Draw detection-only bounding boxes (no text labels) in blue."""
    img_draw = image.copy()
    try:
        from PIL import Image, ImageDraw, ImageFont
        img_pil = Image.fromarray(cv2.cvtColor(img_draw, cv2.COLOR_BGR2RGB))
        ...
        for i, box in enumerate(det_boxes):
            x, y, w, h = box
            draw.rectangle([x, y, x+w, y+h], outline=(0, 100, 255), width=2)
            draw.text((x+2, y+2), f"#{i+1}", font=font, fill=(0, 100, 255))

        overlay_text = f"DET: {model_name}\n{len(det_boxes)} regions"
        overlay_bbox = draw.textbbox((0, 0), overlay_text, font=font)
        draw.rectangle([0, 0, overlay_bbox[2] + 20, overlay_bbox[3] + 20], fill=(0, 0, 0, 180))
        draw.text((10, 10), overlay_text, font=font, fill=(0, 200, 255))
        img_draw = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)
    except Exception as e:
        print(f"  [DET] PIL failed, fallback: {e}")
        for i, (x, y, w, h) in enumerate(det_boxes):
            cv2.rectangle(img_draw, (x, y), (x+w, y+h), (255, 100, 0), 2)
    cv2.imwrite(output_path, img_draw)


def draw_rec_only(image, rec_results, output_path, model_name):
    """Draw recognition results with green boxes and text only."""
    img_draw = image.copy()
    try:
        from PIL import Image, ImageDraw, ImageFont
        img_pil = Image.fromarray(cv2.cvtColor(img_draw, cv2.COLOR_BGR2RGB))
        ...
        for res in rec_results:
            ...
            label = f"{text} ({conf:.2f})" if text else f"[empty] ({conf:.2f})"
            text_bbox = draw.textbbox((0, 0), label, font=font)
            tw = text_bbox[2] - text_bbox[0]
            th = text_bbox[3] - text_bbox[1]
            tx = x
            ty = y - th - 2 if y - th - 2 >= 0 else y + h + 2
            draw.rectangle([tx, ty, tx+tw+4, ty+th+4], fill=(0, 200, 0))
            draw.text((tx+2, ty+2), label, font=font, fill=(0, 0, 0))

        overlay_text = f"REC: {model_name}"
        overlay_bbox = draw.textbbox((0, 0), overlay_text, font=font)
        draw.rectangle([0, 0, overlay_bbox[2] + 20, overlay_bbox[3] + 20], fill=(0, 0, 0, 180))
        draw.text((10, 10), overlay_text, font=font, fill=(0, 255, 100))
        img_draw = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)
    except Exception as e:
        print(f"  [REC] PIL failed, fallback: {e}")
    cv2.imwrite(output_path, img_draw)
...
def main():
    ...
    parser.add_argument(
        "--export-components",
        action="store_true",
        help="For PaddleOCR engines: additionally save separate DET-only and REC-only JSON and annotated images"
    )
    parser.add_argument(
        "--det-model",
        help="Override PaddleOCR detection model (e.g. PP-OCRv6_medium_det). Requires --engine to be a paddleocr engine."
    )
    parser.add_argument(
        "--rec-model",
        help="Override PaddleOCR recognition model (e.g. PP-OCRv5_mobile_rec). Requires --engine to be a paddleocr engine."
    )
    ...
    for engine in engines_to_run:
        ...
        # Determine models (allow --det-model / --rec-model overrides for paddle engines)
        if engine == "paddleocr_v5_mobile":
            det = args.det_model or "PP-OCRv5_mobile_det"
            rec = args.rec_model or "PP-OCRv5_mobile_rec"
            reader = init_paddleocr(paddle_lang, det, rec)
        elif engine == "paddleocr_v5_server":
            det = args.det_model or "PP-OCRv5_server_det"
            rec = args.rec_model or "PP-OCRv5_server_rec"
            reader = init_paddleocr(paddle_lang, det, rec)
        elif engine == "paddleocr_v6_mobile":
            det = args.det_model or "PP-OCRv6_small_det"
            rec = args.rec_model or "PP-OCRv6_small_rec"
            reader = init_paddleocr(paddle_lang, det, rec)
        elif engine == "paddleocr_v6_server":
            det = args.det_model or "PP-OCRv6_medium_det"
            rec = args.rec_model or "PP-OCRv6_medium_rec"
            reader = init_paddleocr(paddle_lang, det, rec)
        ...
        # Detection-only boxes collected for --export-components
        det_boxes = []
        raw_fragments = []
        ...
        if engine.startswith("paddleocr"):
            ...
            for bbox, text, confidence in parsed:
                ...
                det_boxes.append([x, y, width, height])
        ...
        # --- Save OCR results JSON (always) ---
        ocr_json_path = f"ocr_results_{safe_name}.json"
        ocr_json_output = {
            "engine": engine,
            "image": args.image,
            "lang": args.lang,
            "total_time": round(total_time, 3),
            "avg_time_per_bubble": round(avg, 3),
            "regions": results,
        }
        with open(ocr_json_path, "w", encoding="utf-8") as f:
            json.dump(ocr_json_output, f, ensure_ascii=False, indent=2)
        print(f"  Saved OCR results JSON to: {ocr_json_path}")

        # --- Export components (DET + REC separately) if requested ---
        if args.export_components and engine.startswith("paddleocr"):
            # DET-only JSON: bounding boxes without text
            det_json_path = f"{safe_name}_det.json"
            det_json_output = {
                "engine": engine,
                "image": args.image,
                "component": "detection",
                "total_regions": len(det_boxes),
                "regions": [{"bbox": b} for b in det_boxes],
            }
            with open(det_json_path, "w", encoding="utf-8") as f:
                json.dump(det_json_output, f, ensure_ascii=False, indent=2)
            print(f"  [DET] Saved detection JSON to: {det_json_path}")

            # DET annotated image
            det_img_path = f"demo_output_local_{safe_name}_det.jpg"
            draw_det_only(img, det_boxes, det_img_path, engine)
            print(f"  [DET] Saved detection image to: {det_img_path}")

            # REC-only JSON: text + confidence per raw fragment
            rec_json_path = f"{safe_name}_rec.json"
            rec_json_output = {
                "engine": engine,
                "image": args.image,
            ...
            with open(rec_json_path, "w", encoding="utf-8") as f:
                json.dump(rec_json_output, f, ensure_ascii=False, indent=2)
            print(f"  [REC] Saved recognition JSON to: {rec_json_path}")

            # REC annotated image
            rec_img_path = f"demo_output_local_{safe_name}_rec.jpg"
            rec_results_for_draw = [
                {"bbox": [f["x"], f["y"], f["width"], f["height"]], "text": f["text"], "confidence": f["confidence"]}
                for f in raw_fragments
            ]
            draw_rec_only(img, rec_results_for_draw, rec_img_path, engine)
            print(f"  [REC] Saved recognition image to: {rec_img_path}")
    ...
    if len(summary_rows) > 1:
        ...
        print(f"{'=' * 60}")
        header = f"{'Engine':<25} {'OK/Total':<10} {'Errors':<8} {'Total(s)':<12} {'Avg/Bubble(s)':<15}"
        print(header)
        print("-" * len(header))
