...
if [ -z "${API_KEY}" ]; then
  echo "[ERROR] GEMINI_API_KEY not set in ${ENV_FILE} or environment."
  exit 1
fi

echo "[INFO] Testing Google AI Studio (Gemini) with model '${MODEL}'..."

curl -i -X POST https://generativelanguage.googleapis.com/v1beta/openai/chat/completions \
