...
| C02 (`ACTIVE`; [checkpoint](quality-checkpoints/C02.md)) | C01 | Inventory complete: five isolated child packets cover the transaction primitive, layer APIs, logical scene API, archive replacement and global geometry settings. | Text, transform, cleanup, visibility, order and render-setting edits each invalidate output. No edit commits without its revision increment. |
