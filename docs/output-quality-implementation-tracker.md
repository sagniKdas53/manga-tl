...
| [ ] | M5 — independent owners and bounded grouping | H01a; F01–F04 | G1 | G5 | **REVIEW** — [F01](quality-checkpoints/F01.md), [F02](quality-checkpoints/F02.md), and [F03](quality-checkpoints/F03.md) retain partial implementation evidence; completion claims are superseded by the realignment below. H01a has source-feature capture and JSONB storage implementation evidence in [its checkpoint](quality-checkpoints/H01a.md), but remains ACTIVE until an isolated backend callback → stored-region round trip passes. [F04](quality-checkpoints/F04.md) remains BLOCKED on implementation and acceptance evidence, not on full H01 style estimation. |
