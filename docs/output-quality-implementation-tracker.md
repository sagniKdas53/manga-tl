...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A08, source-only A06-C0 and A06-C are `DONE`; A09 and G0 remain `BLOCKED` by missing reviewed holdout ownership/action labels, frozen quality thresholds, and fresh current-checkout stage evidence—not creator, series, uploader, or translator provenance. The [2026-09-13 diversity selection](quality-runs/a09-20260913-holdout/A09-zh-diversity-selection.json) restores `sample201` and `sample205` and supplies eight hash-pinned, visually varied ZH candidates. The [family review](quality-runs/a09-20260913-holdout/A09-family-provenance-review.json) records nine exact Hentoid-to-corpus source-byte matches with four gallery-metadata artist attributions; it remains useful non-blocking source context and near-duplicate evidence.
...
| A09 (`BLOCKED`; [checkpoint](quality-checkpoints/A09.md); [diversity selection](quality-runs/a09-20260913-holdout/A09-zh-diversity-selection.json); [near-duplicate and provenance context](quality-runs/a09-20260913-holdout/A09-family-provenance-review.json)) | A07, A08 | Select a language/layout/style-diverse holdout and freeze the source/label/threshold evaluation manifest under the [coverage requirements](quality-evidence/style-coverage-review.md#anti-overfitting-requirements). Coordinator-reviewed inventory task; no model tuning. | The ZH roster now has eight visually distinct candidate pages, including restored sparse and single-panel controls; A09 remains blocked only until every candidate has holdout-specific ownership/action labels, frozen quality thresholds, and fresh required stage evidence. |
