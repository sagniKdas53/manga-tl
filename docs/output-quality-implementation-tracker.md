...
| A03 (`DONE`; [checkpoint](quality-checkpoints/A03.md)) | A02 | Run the six current source → OCR → translation → cleanup/render baselines in a new run directory, one page at a time. Record real model calls and stage artifacts; no algorithm edit. | Six fresh isolated-stack runs completed with local PP-OCRv6, GPT-5.6 Luna translation/QA LLM, Gemini Flash Lite QA VLM, and fallbacks disabled. The prior disposable series was deleted through the admin API before each next fixture; source/render hashes, resolved settings, and actual QA outcomes are retained. |
