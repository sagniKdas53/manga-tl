...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A07, source-only A06-C0 and A06-C are `DONE`; A08 is next. The coordinator advances later tasks after their dependencies and the applicable entry gate pass. A milestone checkbox means its exit gate has passed, with evidence linked here. An unresolved or unexecuted gate is not a pass.

| Complete | Milestone | Task IDs | Entry condition | Exit gate | Checkpoint |
| --- | --- | --- | --- | --- | --- |
| [ ] | M0 — current baseline and reviewed acceptance data | A01–A09, A06-C | Planning complete | G0 | A01–A07, A06-C0 and A06-C done; A08 next |
...
> Read this tracker, [A07](quality-checkpoints/A07.md), its [coverage audit](quality-runs/a07-20260911-coverage/A07-coverage-audit.json), the [coverage review](quality-evidence/style-coverage-review.md), and relevant source seams. Recheck app/worker/corpus heads. A07 is complete: all 30 fixture packets retain policy/style/protected-art annotations and the unresolved original records as explicit pixel-preserving review. Preserve all retained evidence and original A06 labels. Start A08 as one bounded synthetic-fixture packet; do not regenerate the corpus until G0 passes. After each packet's evidence is reviewed, advance only its dependency-satisfied successor. For later sessions, choose the next dependency-satisfied task, reproduce it on that checkout, run upstream impact, implement only that task and save its checkpoint before stopping. If a behavior already passes, retain a regression check and remove the obsolete fix from the task scope.

Planning checkpoint: all ten milestones and 58 original task cards plus conventional-control child A06-C are specified; A01–A07, source-curation A06-C0 and A06-C are complete, A08 is next, and all milestone gates remain pending. The corpus cutoff and ARM64/new-format scope are integrated. The [original planning validation record](quality-evidence/implementation-plan-validation.json) is historical; the September 10 [coverage review](quality-evidence/style-coverage-review.md) supplements it with the new sample/language requirements. Update the milestone table with checkpoint links as work passes review.
