...
| A03 (`BLOCKED`; [checkpoint](quality-checkpoints/A03.md)) | A02 | Run the six current source → OCR → translation → cleanup/render baselines in a new run directory, one page at a time. Record real model calls and stage artifacts; no algorithm edit. | Blocked: isolated preflight passed, but the current backend/worker stack and a live source-to-stage route are unavailable. Six exact source identities and the non-executed stage manifest are saved; no historical layout replay was substituted. |
