...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A06, source-only A06-C0 and A06-C are `DONE`; A07 is next. The coordinator advances later tasks after their dependencies and the applicable entry gate pass. A milestone checkbox means its exit gate has passed, with evidence linked here. An unresolved or unexecuted gate is not a pass.

| Complete | Milestone | Task IDs | Entry condition | Exit gate | Checkpoint |
| --- | --- | --- | --- | --- | --- |
| [ ] | M0 — current baseline and reviewed acceptance data | A01–A09, A06-C | Planning complete | G0 | A01–A06, A06-C0 and A06-C done; A07 next |
...
> Read this tracker, [A06-C0](quality-checkpoints/A06-C0.md), [A06-C](quality-checkpoints/A06-C.md), the [coverage review](quality-evidence/style-coverage-review.md) and relevant source seams. Recheck app/worker/corpus heads. A06-C is complete: the reported hidden `大GG` plate does not reproduce in its fresh rendered/browser exports, and its upper-left Japanese source is verified without changing immutable baseline data. Preserve all retained evidence and original A06 labels. Start A07 as one fixture per bounded curation packet; do not regenerate the corpus until G0 passes. After each packet's evidence is reviewed, advance only its dependency-satisfied successor. For later sessions, choose the next dependency-satisfied task, reproduce it on that checkout, run upstream impact, implement only that task and save its checkpoint before stopping. If a behavior already passes, retain a regression check and remove the obsolete fix from the task scope.
