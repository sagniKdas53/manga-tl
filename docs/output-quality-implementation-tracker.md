...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A08, source-only A06-C0 and A06-C are `DONE`; A09 and G0 remain `BLOCKED` by unresolved holdout source-family/near-duplicate provenance, the incomplete eight-page ZH roster, and missing holdout labels/thresholds. The four visually reviewed rebuilt ZH candidates in [the 2026-09-13 selection](quality-runs/a09-20260913-holdout/A09-zh-candidate-selection.json) replace the weak `sample201` / `sample205` provisional pair; the [family and near-duplicate review](quality-runs/a09-20260913-holdout/A09-family-provenance-review.json) confirms that adult-content admission does not resolve the provenance gate.

| Complete | Milestone | Task IDs | Entry condition | Exit gate | Checkpoint |
| --- | --- | --- | --- | --- | --- |
| [ ] | M0 — current baseline and reviewed acceptance data | A01–A09, A06-C | G0 blocked source acquisition/review | G0 | A01–A08, A06-C0 and A06-C done; [A09 blocked](quality-checkpoints/A09.md) |
...
| A09 (`BLOCKED`; [checkpoint](quality-checkpoints/A09.md); [candidate selection](quality-runs/a09-20260913-holdout/A09-zh-candidate-selection.json); [family review](quality-runs/a09-20260913-holdout/A09-family-provenance-review.json)) | A07, A08 | Select a language/style/source-family-stratified holdout and freeze the source/label/threshold evaluation manifest under the [coverage requirements](quality-evidence/style-coverage-review.md#anti-overfitting-requirements). Coordinator-reviewed inventory task; no model tuning. | Blocked: four rebuilt ZH pages now satisfy the visual multi-panel/conventional-balloon stratum, and adult material is admissible, but ZH has one proven creator and unverified family clusters; KO has only three independently proven creators; the eight-page ZH roster, holdout labels, and thresholds are not frozen. Do not infer family independence from visual distinction, translator captions, or a non-matching image hash. |
...
| --- | --- |
| G0 — credible baseline | Fresh current-checkout stages for six original and 24 conventional-control source hashes; runtime/font/model provenance; real service preflight; reviewed labels and language/style/family-stratified held-out IDs. Unknown mappings/timestamps/languages remain unknown. |
...
> Read this tracker, [A07](quality-checkpoints/A07.md), its [coverage audit](quality-runs/a07-20260911-coverage/A07-coverage-audit.json), [A08](quality-checkpoints/A08.md), its [fixtures](quality-evidence/a08-pdf-derived-scene-fixtures.json), [A09](quality-checkpoints/A09.md), its [blocked inventory](quality-runs/a09-20260911-holdout/A09-holdout-inventory.json), the [coverage review](quality-evidence/style-coverage-review.md), and relevant source seams. Recheck app/worker/corpus heads. A07 is complete: all 30 fixture packets retain policy/style/protected-art annotations and the unresolved original records as explicit pixel-preserving review. A08 is complete: five measurable synthetic source-space scenes retain fractional rotation, blank/manual, padding/handle, narrow-owner, and cleanup-overlap requirements without treating PDF screenshots as editable geometry. A09 is blocked: only two eligible ZH multi-panel conventional-balloon candidates remain, and labels/thresholds/family provenance cannot yet freeze a holdout. Preserve all retained evidence and original A06 labels. Acquire and review qualifying independent ZH sources before resuming A09; do not regenerate the corpus until G0 passes. After each packet's evidence is reviewed, advance only its dependency-satisfied successor. For later sessions, choose the next dependency-satisfied task, reproduce it on that checkout, run upstream impact, implement only that task and save its checkpoint before stopping. If a behavior already passes, retain a regression check and remove the obsolete fix from the task scope.

Planning checkpoint: all ten milestones and 58 original task cards plus conventional-control child A06-C are specified; A01–A08, source-curation A06-C0 and A06-C are complete, A09 and G0 are blocked pending qualifying independent ZH source acquisition plus holdout labels/thresholds/family review, and all milestone gates remain pending. The corpus cutoff and ARM64/new-format scope are integrated. The [original planning validation record](quality-evidence/implementation-plan-validation.json) is historical; the September 10 [coverage review](quality-evidence/style-coverage-review.md) supplements it with the new sample/language requirements. Update the milestone table with checkpoint links as work passes review.
