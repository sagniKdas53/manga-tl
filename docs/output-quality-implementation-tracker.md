...
Planning checkpoint: 2026-09-09; source-coverage update: 2026-09-10. **Implementation is in progress:** A01–A06 and source-curation child A06-C0 are complete; fresh conventional-control baselines/labels (A06-C), M0 and every later milestone remain open. This is the authoritative execution tracker for the user's requested unified renderer, SFX policy, independent text ownership and generated cleanup improvements. Read the [architecture recommendations](output-quality-architecture-decisions.md) for rationale and the [investigation](output-quality-investigation.md) for reproductions.
...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A08, source-only A06-C0 and A06-C are `DONE`. A09 and G0 remain `BLOCKED`, but source acquisition is no longer the blocker: the [candidate roster](quality-runs/a09-20260913-holdout/A09-candidate-roster-20260913.json) carries ten coordinator-reviewed pages per language, above the eight-page minimum. Remaining blockers are holdout-specific ownership/action labels, frozen quality thresholds, fresh current-checkout stage evidence for promoted pages, and English references for the pages that have no human translation. Creator, series, uploader and translator provenance is recorded context and gates nothing.

Current A09 roster state. Coordinator review rejected `sample253` (625x587, trivial layout, source text already overflowing its bubble) and confirmed `sample229` as a deliberately simple but visually unique ZH control. The 2026-09-13 X-scrape archive contributed 33 image-backed source posts (16 KO, 9 JA, 8 ZH) and closed the Korean reserve gap that the Hentoid and Pixiv collections could not fill. Promoted: `ko-x4`, `ko-x14`, `ko-x15`, `ja-x8`, one Hentoid JA page, `zh-x4` and `zh-x7`. Review sheets and their tracked [sheet index](quality-evidence/candidate-sheets-20260913/sheet-index.json) support confirmation and replacement; the [family review](quality-runs/a09-20260913-holdout/A09-family-provenance-review.json) records nine exact Hentoid-to-corpus source-byte matches with four gallery-metadata artist attributions.

| Complete | Milestone | Task IDs | Entry condition | Exit gate | Checkpoint |
| --- | --- | --- | --- | --- | --- |
| [ ] | M0 — current baseline and reviewed acceptance data | A01–A09, A06-C | G0 blocked on holdout labels/thresholds/stage evidence | G0 | A01–A08, A06-C0 and A06-C done; [A09 blocked](quality-checkpoints/A09.md) with a ten-page-per-language roster selected |
...
| A09 (`BLOCKED`; [checkpoint](quality-checkpoints/A09.md); [candidate roster](quality-runs/a09-20260913-holdout/A09-candidate-roster-20260913.json); [ZH diversity selection](quality-runs/a09-20260913-holdout/A09-zh-diversity-selection.json); [near-duplicate and provenance context](quality-runs/a09-20260913-holdout/A09-family-provenance-review.json)) | A07, A08 | Select a language/layout/style-diverse holdout and freeze the source/label/threshold evaluation manifest under the [coverage requirements](quality-evidence/style-coverage-review.md#anti-overfitting-requirements). Coordinator-reviewed inventory task; no model tuning. | Ten reviewed candidate pages exist per language, with reserves for JA/KO/ZH replacements. A09 stays blocked until every selected page is hash-pinned and source-reviewed, carries holdout-specific ownership/action labels and frozen thresholds, has fresh current-checkout stage evidence, and has an English reference — human where available, otherwise a Torii API reference that is never treated as a translation pass. |

G0 checkpoint: six original fresh baselines plus the 24 conventional-control baselines/labels, machine/environment provenance, real integration preflight, reviewed evaluation manifest and unresolved hypotheses. The [coverage audit](quality-evidence/style-coverage-review.md) records declared language counts, wrong-language exclusions, exact duplicates and remaining style gaps; it is not a G0 pass. If model/services are unavailable, stop dependent quality work at a saved checkpoint; do not silently substitute old OCR.
...
| J02 | J01 | Evaluate the frozen A09 holdout without tuning on its labels. Include conventional bubble/panel pages and strips in JA/KO/ZH as well as dense/colored pages, decorated dialogue, SFX, thin strokes, close boxes and large images. | At least 24 independent held-out pages, 8 per language, with A09 layout/style quotas verified. Per-language/style failures cannot be hidden by the aggregate. Any failure returns to the responsible task with a new regression; evaluate another reserved set after tuning. |
...
These are proposed acceptance requirements to freeze with A09; they are not measured achievements. Six curated cases are hard regression gates; the 24 conventional controls are additional required checks, not replacements. Report controls by language/style separately from the adversarial regressions and independent holdout. Larger-set scores cannot hide a failing curated case or conventional-page slice in an average.
...
> Read this tracker, [A07](quality-checkpoints/A07.md), its [coverage audit](quality-runs/a07-20260911-coverage/A07-coverage-audit.json), [A08](quality-checkpoints/A08.md), its [fixtures](quality-evidence/a08-pdf-derived-scene-fixtures.json), [A09](quality-checkpoints/A09.md), its [candidate roster](quality-runs/a09-20260913-holdout/A09-candidate-roster-20260913.json) and [sheet index](quality-evidence/candidate-sheets-20260913/sheet-index.json), the [coverage review](quality-evidence/style-coverage-review.md), and relevant source seams. Recheck app/worker/corpus heads. A07 is complete: all 30 fixture packets retain policy/style/protected-art annotations and the unresolved original records as explicit pixel-preserving review. A08 is complete: five measurable synthetic source-space scenes retain fractional rotation, blank/manual, padding/handle, narrow-owner, and cleanup-overlap requirements without treating PDF screenshots as editable geometry. A09 has ten reviewed candidate pages per language plus JA/KO/ZH reserves; it is blocked only by hash pinning and source review of promoted pages, holdout labels, frozen thresholds, fresh stage evidence, and missing English references. Preserve all retained evidence and original A06 labels. Do not regenerate the corpus until G0 passes. After each packet's evidence is reviewed, advance only its dependency-satisfied successor. For later sessions, choose the next dependency-satisfied task, reproduce it on that checkout, run upstream impact, implement only that task and save its checkpoint before stopping. If a behavior already passes, retain a regression check and remove the obsolete fix from the task scope.

Planning checkpoint: all ten milestones and 58 original task cards plus conventional-control child A06-C are specified; A01–A08, source-curation A06-C0 and A06-C are complete, and A09/G0 are blocked only on holdout-specific quality work: hash pinning and source review of promoted pages, reviewed ownership/action labels, frozen thresholds, fresh-stage evidence, and English references for pages without a human translation. Ten candidate pages are selected per language, with JA/KO/ZH reserves from the Hentoid, Pixiv and 2026-09-13 X-scrape collections; creator/series provenance remains optional context. All milestone gates remain pending. The corpus cutoff and ARM64/new-format scope are integrated. The [original planning validation record](quality-evidence/implementation-plan-validation.json) is historical; the September 10 [coverage review](quality-evidence/style-coverage-review.md) supplements it with the new sample/language requirements. Update the milestone table with checkpoint links as work passes review.
