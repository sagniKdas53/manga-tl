...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A06 and source-only A06-C0 are `DONE`; A06-C is next for the newly selected controls, before A07 can complete. The coordinator advances later tasks after their dependencies and the applicable entry gate pass. A milestone checkbox means its exit gate has passed, with evidence linked here. An unresolved or unexecuted gate is not a pass.

| Complete | Milestone | Task IDs | Entry condition | Exit gate | Checkpoint |
| --- | --- | --- | --- | --- | --- |
| [ ] | M0 — current baseline and reviewed acceptance data | A01–A09, A06-C | Planning complete | G0 | A01–A06 and A06-C0 done; A06-C next |
...
| A06-C (`ACTIVE`; [source selection A06-C0](quality-checkpoints/A06-C0.md); [review checkpoint](quality-checkpoints/A06-C.md)) | A06 | Add fresh A03/A04-style baseline/editor/export evidence and A06-style owner labels for the [24 conventional controls](quality-evidence/style-coverage-20260910/resolved-selection.json), one fixture per bounded packet. Existing six baselines remain immutable; no whole-corpus regeneration. | All 24 source hashes and 228 final OCR boxes have reviewed source ownership: 170 dialogue owners, 47 non-dialogue elements, and 11 free-standing captions. Live fragment capture resolves `sample134-r001` as SFX and confirms `sample261-r008` correctly groups six fragments in one speech balloon. A narrow direct-text eligibility guard removes three `sample47` false-positive plates, but the grey `Not bad.` plate still renders over SFX despite a hidden/empty saved translation element; the upper-left OCR remains uncorrected. A06-C is gate-blocked until that behavior is traced and fresh evidence passes. |
| A07 | A06, A06-C | Prepare region-action, glyph-support and source-style annotations for the reviewed owners across the six originals and 24 conventional controls, one fixture per bounded curation packet. Luna drafts; coordinator reviews pixels and labels. | Required dialogue/SFX, outlined glyph, protected-art and style cases plus conventional bubble/panel controls have reviewed labels in JA/KO/ZH. Unsupported/unlabelled regions remain coverage gaps. |
...
| A09 | A07, A08 | Select a language/style/source-family-stratified holdout and freeze the source/label/threshold evaluation manifest under the [coverage requirements](quality-evidence/style-coverage-review.md#anti-overfitting-requirements). Coordinator-reviewed inventory task; no model tuning. | At least 24 separate held-out pages, 8 JA / 8 KO / 8 ZH; at least 4 multi-panel conventional-balloon pages per language, including 4 B&W JA pages. Verify source language, exclude translated/wrong-language inputs and near-duplicate/related development families. Required unverified labels or unavailable independent strata block freezing; do not backfill with more illustrations. |
...
> Read this tracker, [A06-C0](quality-checkpoints/A06-C0.md), [A06-C](quality-checkpoints/A06-C.md), the [coverage review](quality-evidence/style-coverage-review.md) and relevant source seams. Recheck app/worker/corpus heads. A06-C has complete source-owner labels but remains blocked by `sample47`: trace why an empty, hidden `大GG` translation element still renders as a grey `Not bad.` plate, then verify the upper-left Japanese source before any OCR correction. Preserve all retained evidence and original A06 labels. Do not start A07 or regenerate the corpus until A06-C passes. After each packet's evidence is reviewed, advance only its dependency-satisfied successor. For later sessions, choose the next dependency-satisfied task, reproduce it on that checkout, run upstream impact, implement only that task and save its checkpoint before stopping. If a behavior already passes, retain a regression check and remove the obsolete fix from the task scope.

Planning checkpoint: all ten milestones and 58 original task cards plus conventional-control child A06-C are specified; A01–A06 and source-curation A06-C0 are complete and all milestone gates remain pending. The corpus cutoff and ARM64/new-format scope are integrated. The [original planning validation record](quality-evidence/implementation-plan-validation.json) is historical; the September 10 [coverage review](quality-evidence/style-coverage-review.md) supplements it with the new sample/language requirements. Update the milestone table with checkpoint links as work passes review.
