...
| --- | --- | --- | --- |
| A01 (`READY`) | — | Add a fail-fast local quality-run preflight and isolated test-service recipe under `scripts/` and `docs/quality-checkpoints/`; inventory actual heads, containers, model/font hashes and hardware. Use existing Compose definitions only as inputs. | Missing DB/Redis/storage fails preflight. Insert/read a sentinel in an isolated test DB; record actual service connectivity without secrets. No production DB mutation. |
| A02 | A01 | Add a capture/replay harness around current OCR/grouping stage seams (`worker/` test tooling plus a narrowly bounded hook if needed). Save raw quads, scale transforms, detector masks, recognition, grouping edges and final owners; no algorithm change. | One source replays the captured stage twice with equal stable IDs and geometry. State which paths are live, cached or stubbed. |
| A03 | A02 | Run the six current source → OCR → translation → cleanup/render baselines in a new run directory, one page at a time. Record real model calls and stage artifacts; no algorithm edit. | Six stage manifests with exact source/runtime hashes. Reuse translation only with verified source/region mapping; old-layout replay alone does not pass. |
| A04 | A03 | Capture actual editor and PNG/ZIP outputs for the six baseline projects using existing `scripts/playwright/` tooling. | Saved browser/export images, artifact revisions and effective fonts; no helper-only substitutes. |
| A05 | A04 | Produce comparison sheets and the fixed/remaining/not-reproduced issue matrix from A03/A04. Evidence/report task only. | Coordinator reviews each reported defect before scheduling its fix; service/model availability gaps remain explicit. |
| A06 | A05 | Prepare independent owner/container/fragment labels for the six exact source images in evaluation data. Luna drafts; coordinator visually reviews assignments. | Six illustration labels in sample177 have distinct ownership; no ambiguous crop or positional guess marked verified. |
