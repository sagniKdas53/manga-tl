...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A06, source-only A06-C0 and A06-C are `DONE`; A07 is `ACTIVE` with reviewed `sample7` and `sample150` packets. The coordinator advances later tasks after their dependencies and the applicable entry gate pass. A milestone checkbox means its exit gate has passed, with evidence linked here. An unresolved or unexecuted gate is not a pass.

| Complete | Milestone | Task IDs | Entry condition | Exit gate | Checkpoint |
| --- | --- | --- | --- | --- | --- |
| [ ] | M0 — current baseline and reviewed acceptance data | A01–A09, A06-C | Planning complete | G0 | A01–A06, A06-C0 and A06-C done; A07 active with sample7/sample150 review |
...
| A07 (`ACTIVE`; [sample7 packet](quality-checkpoints/A07-sample7.md); [sample150 packet](quality-checkpoints/A07-sample150.md)) | A06, A06-C | Prepare region-action, glyph-support and source-style annotations for the reviewed owners across the six originals and 24 conventional controls, one fixture per bounded curation packet. Luna drafts; coordinator reviews pixels and labels. | `sample7` provides reviewed JA conventional-balloon, preserve-SFX, protected-art and source-style labels for all 24 owners: 14 replace candidates and 10 preserve decisions. `sample150` adds eight colored-strip dialogue candidates, an explain-only title, and an outlined-glyph protected-art `review` case. Required dialogue/SFX, outlined glyph, protected-art and style cases plus conventional bubble/panel controls must still have reviewed labels in JA/KO/ZH. Unsupported/unlabelled regions remain coverage gaps. |
...
> Read this tracker, [A06-C0](quality-checkpoints/A06-C0.md), [A06-C](quality-checkpoints/A06-C.md), [A07-sample7](quality-checkpoints/A07-sample7.md), [A07-sample150](quality-checkpoints/A07-sample150.md), the [coverage review](quality-evidence/style-coverage-review.md) and relevant source seams. Recheck app/worker/corpus heads. A06-C is complete: the reported hidden `大GG` plate does not reproduce in its fresh rendered/browser exports, and its upper-left Japanese source is verified without changing immutable baseline data. A07 is active: `sample7` supplies a reviewed JA conventional-balloon/preserve-SFX packet, while `sample150` supplies the outlined-glyph protected-art review case. Preserve all retained evidence and original A06 labels. Continue A07 as one fixture per bounded curation packet, prioritizing KO and ZH source-style cases; do not regenerate the corpus until G0 passes. After each packet's evidence is reviewed, advance only its dependency-satisfied successor. For later sessions, choose the next dependency-satisfied task, reproduce it on that checkout, run upstream impact, implement only that task and save its checkpoint before stopping. If a behavior already passes, retain a regression check and remove the obsolete fix from the task scope.

Planning checkpoint: all ten milestones and 58 original task cards plus conventional-control child A06-C are specified; A01–A06, source-curation A06-C0 and A06-C are complete, A07 is active, and all milestone gates remain pending. The corpus cutoff and ARM64/new-format scope are integrated. The [original planning validation record](quality-evidence/implementation-plan-validation.json) is historical; the September 10 [coverage review](quality-evidence/style-coverage-review.md) supplements it with the new sample/language requirements. Update the milestone table with checkpoint links as work passes review.
