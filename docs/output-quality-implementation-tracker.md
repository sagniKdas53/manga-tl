...
| C02 (`ACTIVE`; [checkpoint](quality-checkpoints/C02.md)) | C01 | Five child packets cover the transaction primitive, layer APIs, logical scene API, archive replacement and global geometry settings. [C02-R1](quality-checkpoints/C02-R1.md) and [C02-R2](quality-checkpoints/C02-R2.md) are DONE; C02-R3 is READY. | Text, transform, cleanup, visibility, order and render-setting edits each invalidate output. No edit commits without its revision increment. |
