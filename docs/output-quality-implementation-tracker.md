...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A09, source-only A06-C0 and A06-C are `DONE`. A09 froze on 2026-09-13: [holdout manifest](quality-runs/a09-20260913-holdout/A09-holdout-manifest.json), 30 pages at 10 per language, 346 reviewed ownership/action labels, [frozen thresholds](quality-runs/a09-20260913-holdout/A09-quality-thresholds.json) derived only from development controls, and [fresh stage evidence](quality-runs/a09-20260913-stages/stage-manifest.json) for 77 pages. G0 now needs only its own checkpoint write-up: the fresh six-original and 24-control stages, runtime/font/model provenance and service preflight already exist, and no pipeline code changed since that evidence.
...
| --- | --- | --- | --- | --- | --- |
| [x] | M0 — current baseline and reviewed acceptance data | A01–A09, A06-C | A01–A09 complete | G0 | A01–A09, A06-C0 and A06-C done; [A09 frozen](quality-checkpoints/A09.md); [G0 passed](quality-checkpoints/G0.md) |
...
| A09 (`DONE`; [checkpoint](quality-checkpoints/A09.md); [holdout manifest](quality-runs/a09-20260913-holdout/A09-holdout-manifest.json); [reviewed labels](quality-runs/a09-20260913-holdout/A09-holdout-labels.reviewed.json); [thresholds](quality-runs/a09-20260913-holdout/A09-quality-thresholds.json); [stage run](quality-runs/a09-20260913-stages/stage-manifest.json)) | A07, A08 | Select a language/layout/style-diverse holdout and freeze the source/label/threshold evaluation manifest under the [coverage requirements](quality-evidence/style-coverage-review.md#anti-overfitting-requirements). Coordinator-reviewed inventory task; no model tuning. | 30 frozen pages, 10 per language, with 346 reviewed labels (226 replace, 55 preserve, 31 explain, 34 unresolved review), thresholds frozen from development controls only, fresh stage evidence for all 77 roster and reserve pages, and zero development overlap by ID or source bytes. No holdout page is scored; J02 evaluates. |

G0 checkpoint ([passed 2026-09-13](quality-checkpoints/G0.md)): six original fresh baselines plus the 24 conventional-control baselines/labels, machine/environment provenance, real integration preflight, reviewed evaluation manifest and unresolved hypotheses. Measured populations are separated: the 24 controls give the ordinary-case baseline that thresholds derive from, and the six originals stay the defect population. The [coverage audit](quality-evidence/style-coverage-review.md) records declared language counts, wrong-language exclusions, exact duplicates and remaining style gaps; it is not a G0 pass. If model/services are unavailable, stop dependent quality work at a saved checkpoint; do not silently substitute old OCR.
