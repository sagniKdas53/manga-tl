...
| R5 — provider hygiene | Pages stop failing for config reasons. | `config/providers.json`: qaVLM model for `neurometric`; `clawpack` pricing; NSFW-half translation model (user decision). | No `no longer valid` / `Cost unknown` warnings in a six-fixture run; page 1 of a run gets VLM QA like the others. |

Order: R4 first (one packet, unblocks measurement — done; every later checkpoint includes its `reference-compare.md`), then R1, R2, R3, R5 alongside. UR03 is retargeted: show the six fixtures after R2 next to source, August baseline and Torii. M7 typography and M9 corpus work wait for R3.
...
> Immediate task (2026-09-17): follow the [realignment](#2026-09-17-realignment--one-live-path). R4 is done; next is R1 (render cutover), starting with `impact()` on `process_pending_renders`, `enqueue_current_snapshot_render`, `handle_render_callback` and the translation-callback layer writer. No task closes on tests or an isolated harness; every checkpoint shows the six fixtures next to source, August baseline and Torii. Preserve frozen G0/A09 evidence; do not score A09, regenerate the corpus or deploy.
