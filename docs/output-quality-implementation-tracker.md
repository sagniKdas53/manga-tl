...
Planning checkpoint: 2026-09-09; G0 evidence review: 2026-09-13. **Implementation is in progress:** A01–A08, source-curation child A06-C0 and A06-C retain their completed historical work; A09 acceptance protocol and G0 are `REVIEW`. M0 is not passed and M1–M9 remain pending. The immediate task is [G0 evidence closure](quality-checkpoints/G0-terra-handoff.md), not pipeline quality fixes. This is the authoritative execution tracker for the user's requested unified renderer, SFX policy, independent text ownership and generated cleanup improvements. Read the [architecture recommendations](output-quality-architecture-decisions.md) for rationale and the [investigation](output-quality-investigation.md) for reproductions.
...
Task statuses: `TODO`, `READY`, `ACTIVE`, `REVIEW`, `DONE`, `BLOCKED`. A01–A08, source-only A06-C0 and A06-C retain their completed historical work. A09's [roster and manifest](quality-runs/a09-20260913-holdout/A09-holdout-manifest.json) are preserved: 30 pages at 10 per language, 346 reviewed labels and [retained stage evidence](quality-runs/a09-20260913-stages/stage-manifest.json) for 77 roster/reserve pages. **A09 acceptance protocol and G0 are `REVIEW`:** the original baseline predates a worker behavior change; proxy metrics and font provenance were overstated; threshold approval/enforcement and the holdout evaluation procedure need correction. This requires evidence/tooling work, not just a checkpoint write-up. Follow the [review findings](quality-checkpoints/G0.md) and [Terra completion handoff](quality-checkpoints/G0-terra-handoff.md). Do not overwrite the frozen artifacts or reopen source acquisition.
...
| [ ] | M1 — new artifact and API contract | B01–B05 | G0 | G1 | **READY** — begin B01 from the frozen G0 baseline; preserve A09 and explicit unresolved identities |
...
G0 checkpoint ([REVIEW; previous pass withdrawn](quality-checkpoints/G0.md)): requires a defensible current baseline for all six originals and 24 controls, actual runtime/font/model provenance, real isolated-service evidence, reviewed evaluation inputs and an approved evaluation protocol. The retained files are valid historical evidence, not automatically current-checkout evidence. Keep development-control measurements separate from the six defect cases. Known bad output and explicit unresolved labels are baseline findings, not instructions to implement M1–M8 early. Follow the [bounded closure handoff](quality-checkpoints/G0-terra-handoff.md); stop dependent paid runs on unavailable services/models while continuing independent tooling/documentation work. Do not silently substitute old OCR, lower later quality requirements, score the holdout or regenerate the corpus.
...
> Immediate task: start M1 from the G0 eligibility/source-identical-export findings in [G0 review](quality-checkpoints/G0.md). Preserve the frozen G0 baseline, A09 roster/reserves, and all explicit unresolved identities. Do not score A09, regenerate the corpus, or begin B01.
