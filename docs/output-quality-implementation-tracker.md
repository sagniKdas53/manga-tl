...
| A09 (`BLOCKED`; [checkpoint](quality-checkpoints/A09.md); [inventory](quality-runs/a09-20260911-holdout/A09-holdout-inventory.json)) | A07, A08 | Select a language/style/source-family-stratified holdout and freeze the source/label/threshold evaluation manifest under the [coverage requirements](quality-evidence/style-coverage-review.md#anti-overfitting-requirements). Coordinator-reviewed inventory task; no model tuning. | Blocked: only two independent candidate ZH multi-panel conventional-balloon pages remain after development/family exclusions; four are required. Candidate labels, thresholds, and most family provenance also remain unverified. Do not backfill with related, translated, wrong-language, single-panel, or illustration inputs. |
