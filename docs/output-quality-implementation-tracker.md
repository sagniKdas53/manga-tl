...
| A06-C (`READY`; [source selection A06-C0](quality-checkpoints/A06-C0.md)) | A06 | Add fresh A03/A04-style baseline/editor/export evidence and A06-style owner labels for the [24 conventional controls](quality-evidence/style-coverage-20260910/resolved-selection.json), one fixture per bounded packet. Existing six baselines remain immutable; no algorithm change or whole-corpus regeneration. | All 24 source hashes accounted for with current-checkout stage provenance and reviewed owner/container labels. Ordinary white-bubble interiors, borders, gutters and adjacent art explicitly checked. Failures/unresolved owners remain blocking. |
...
> Read this tracker, [A06-C0](quality-checkpoints/A06-C0.md), the [coverage review](quality-evidence/style-coverage-review.md) and relevant source seams. Recheck app/worker/corpus heads. Start **A06-C** with one conventional fixture (`sample7`) from the hashed selection, capturing fresh isolated-stack baseline/export evidence and owner labels; preserve all unresolved original A06 labels. Continue A07 over both the six originals and new controls after the required per-fixture evidence exists. Do not implement rendering/grouping fixes or regenerate the corpus. After each packet's evidence is reviewed, advance only its dependency-satisfied successor. For later sessions, choose the next dependency-satisfied task, reproduce it on that checkout, run upstream impact, implement only that task and save its checkpoint before stopping. If a behavior already passes, retain a regression check and remove the obsolete fix from the task scope.
