...
| R5 — provider hygiene | Pages stop failing for config reasons. | `config/providers.json`: qaVLM model for `neurometric`; `clawpack` pricing; NSFW-half translation model (user decision). Also from the R1 live run (2026-09-18): `recovery.rs` re-queues a `PROCESSING` translation job after ~10 minutes with no heartbeat, so `sample61`'s 27-chunk DeepSeek translation ran as three concurrent copies (attempts 1–3) and all three callbacks were accepted — a long job needs a heartbeat or a per-type stale window, and a callback for a superseded attempt must be dropped. `capture_quality_baseline.cjs` treats *any* active job as "not idle" and can only `--register` on an empty database (second registration must pass `role: translator`; see `capture_existing_page.cjs`). | No `no longer valid` / `Cost unknown` warnings in a six-fixture run; page 1 of a run gets VLM QA like the others. |
