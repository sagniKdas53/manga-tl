...
> identifies as mangatranslator.ai running qwen3-235b) would mean grading models against a model.
> Order: human → manually-edited → mangatranslator.ai → mangatranslate.com → mangatranslate.online → google-lens → local.
> (These are literal `provenance` values in `meta.json`; the on-disk vocabulary still spells the
> vendor names out — see the corpus rename task.)
