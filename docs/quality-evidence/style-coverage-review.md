...
| ZH | `sample641` | **Added 2026-09-19 by user decision (25th control).** User-curated simplified-Chinese B&W panel page from `corpus/gaps/pending/zh/` with a human English reference (`ref-curated-en.webp`) and a Torii bundle; conventional multi-column balloons. First page on which `AUDIT-R20` (balloon split into per-column regions) was seen live. |

The original 24 additions (before `sample641`) contain **20 panel/strip pages**, **3 single-panel balloon pages**, and **1 scene collage**. Nineteen are tagged with clean gutters; the action page is separate. Nine are B&W panel pages, three B&W single-panel pages, one limited-palette page, and eleven color pages. All 24 contain conventional speech balloons, although some also contain free-standing dialogue, narration or SFX. This is deliberate mixed-case coverage, not a claim that every text region has a white bubble.
...
| Traditional serialized/print manhua, especially dense B&W or ink-heavy action | **No visually reviewed representative selected in this pass.** Chinese-language images mostly establish translated/fan-webcomic styles, not a broad print-manhua style slice. | Obtain visually distinct conventional Chinese narrative pages when that style is needed; do not relabel a Chinese-translated manga page as traditional manhua. |
...
| Physical print-scan damage | **No visually reviewed representative selected.** `sample138` is degraded, but its 358x506 raster and uncertain language cannot establish a print-scan quality slice. | Binding curvature, bleed-through, paper aging and scan moire remain unvalidated. Keep separate from the requested clean-digital baseline. |
...
| Chinese/Korean visual diversity | The pending ZH sheets contain a repeated flat-color two-panel fancomic template, while KO also repeats several visual templates and includes wrong-language variants. | Equal language counts alone would still be biased. Deliberately select distinct layouts, palettes, dialogue densities, and text treatments; record source provenance when it is available, but do not block quality evaluation on it. |
...
4. Review exact and visual near-duplicates before splitting. No same original page, alternate-language rendering, adjacent chapter episode, or known direct development-relative page may cross development/holdout. Require at least four visibly distinct layout/art-style clusters per language and no more than two pages from a closely repeated visual template. Missing creator provenance does not make a page ineligible.
5. Keep `sample258`/`sample637` together; review `sample361`/`sample363`; group `sample123` with related `sample122`/`sample124`/`sample125`, the `sample208`/`sample209` pair and related `sample226`/`sample227`/`sample230`/`sample231` pages. Check visually repetitive candidate groups before selecting. These examples are not a complete visual-cluster map.
6. Do not tune on holdout outputs or labels. If the corpus cannot supply correctly labelled language/layout/style coverage after exclusions, **A09 remains blocked and requests additional sources**. Do not backfill with English-translated sources, direct duplicates, related pages or more free-standing illustration text simply to reach 24.
