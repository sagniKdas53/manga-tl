...
> comes from that sample's `meta.json`, generated by `scripts/flatten_samples.py` from the
> committed `scripts/examples_manifest.json` — not from filename sniffing. The old heuristics
...
1. Reads the source image and the reference renders from the sample's `meta.json`, then picks
   ...
   `scripts/examples_manifest.json`.
